...
class SidebarOrganizer {
    constructor() {
        ...
        [HA_EVENT.SETTHEME, HA_EVENT.DEFAULT_PANEL, HA_EVENT.DIALOG_CLOSED, HA_EVENT.LOCATION_CHANGED].forEach(
            (event) => {
    ...
    private _handleBottomPanels(bottomItems: string[]) {
        ...
        const divider = this.sideBarRoot!.querySelector(
            `${SELECTOR.DIVIDER}:not([added]):not([ungrouped])`
        ) as HTMLElement;
    ...
    private _addConfigDialog() {
        ...
        const setCodeUILabel = () => {
            const code =
                this._sidebarDialog?._tabState === TAB_STATE.CODE
                    ? BTN_LABEL.SHOW_VISUAL_EDITOR
                    : BTN_LABEL.SHOW_CODE_EDITOR;
            const codeBtn = haDialog.querySelector(
                `${ELEMENT.HA_BUTTON}[slot=${SLOT.SECONDARY_ACTION}]`
            ) as HTMLElement;
    ...
    private _createNewItem(itemConfig: NewItemConfig): SidebarPanelItem {
        ...
        item.innerHTML = `
            <span class="item-text" slot="headline">${itemConfig.title}</span>
        `;
